...
Vue.component("chart", {
  ...
  template:
    "<div><h1>CHART {{title}}</h1><pre>{{JSON.stringify(chart_data, null, 2)}}</pre></div>",
  mounted: function () {
...
Vue.component("units", {
  ...
  template:
    '<span><span v-if="units==\'us\'" v-html="us_content"></span><span v-else v-html="metric_content"></span></span>',
  mounted: function () {},
  methods: {
      ...
      for (const r of replacements) {
        ...
        while (i < content.length) {
          if (content[i] == r.token) {
            if (started) {
              content =
                content.slice(0, i) + `</${r.tag}>` + content.slice(i + 1);
              started = false;
            } else {
              content =
                content.slice(0, i) + `<${r.tag}>` + content.slice(i + 1);
...
Vue.component("converter", {
  ...
  mounted: function () {
    const v = this;
    axios
      .get("/statics/unit-conversions.json")
  ...
  methods: {
    ...
    recalculate: function () {
      const special =
        isNaN(this.unit_from.factor) || isNaN(this.unit_to.factor);
      if (special) {
        ...
        this.to = (standard * this.unit_to.factor).toPrecision(
          this.sig_fig_output,
        );
      }
    },
...
Vue.component("viscosity-converter", {
  ...
  methods: {
    ...
    dd() {
      ...
      const centipoise = (input * this.from_unit.toPrime).toPrecision(
        this.output_sig_fig,
      );
      if (this.from_unit.id != 1)
        this.steps.push(
          `${input} ${this.from_unit.label} x ${this.from_unit.toPrime} = ${centipoise} Centipoise`,
        );

      const output = (centipoise / this.to_unit.toPrime).toPrecision(
        this.output_sig_fig,
      );
      if (this.to_unit.id != 1)
        this.steps.push(
          `${centipoise} Centipoise / ${this.to_unit.toPrime} = ${output} ${this.to_unit.label}`,
        );
    ...
    dk() {
      ...
      const centistoke = (centipoise / parseFloat(this.sg)).toPrecision(
        this.output_sig_fig,
      );
      this.steps.push(
        `${centipoise} Centipoise / ${this.sg} = ${centistoke} Centistoke`,
      );
      ...
      if (this.to_unit.toPrime === "ssu100") {
        ...
        output = (centistoke / this.to_unit.toPrime).toPrecision(
          this.output_sig_fig,
        );
      }
      if (this.to_unit.id != 5)
        this.steps.push(
          `${centistoke} Centistoke / ${this.to_unit.toPrime} = ${output} ${this.to_unit.label}`,
        );
      this.to_value = output;

      if (
        this.to_unit.hasOwnProperty("cSt_cuttoff") &&
        this.to_unit.cSt_cuttoff_max &&
        centistoke > this.to_unit.cSt_cuttoff_max
      ) {
      ...
      if (
        this.to_unit.hasOwnProperty("cSt_cuttoff_min") &&
        this.to_unit.cSt_cuttoff_min &&
        centistoke < this.to_unit.cSt_cuttoff_min
      ) {
    ...
    kk() {
      ...
      //-----------------------------------
      // Find significant figures of input
      const centistoke = (input * this.from_unit.toPrime).toPrecision(
        this.output_sig_fig,
      );
      if (this.from_unit.id != 5)
        this.steps.push(
          `${input} ${this.from_unit.label} x ${this.from_unit.toPrime} = ${centistoke} Centistoke`,
        );
    ...
    kd() {
      ...
      if (
        this.to_unit.hasOwnProperty("cSt_cuttoff") &&
        this.to_unit.cSt_cuttoff &&
        centistoke > this.to_unit.cSt_cuttoff
      ) {
      ...
      const centipoise = (input * parseFloat(this.sg)).toPrecision(
        this.output_sig_fig,
      );
      this.steps.push(
        `${input} Centistokes * ${this.sg} = ${centipoise} Centipoise`,
      );
      const output = (centipoise / this.to_unit.toPrime).toPrecision(
        this.output_sig_fig,
      );
    ...
    calculate() {
      ...
      if (this.to_unit && this.from_unit) {
        if (this.from_unit.category == "D" && this.to_unit.category == "D") {
          ...
          this.from_unit.category == "D" &&
          this.to_unit.category == "K"
        ) {
          this.dk();
        } else if (
          this.from_unit.category == "K" &&
          this.to_unit.category == "K"
        ) {
          this.kk();
        } else if (
          this.from_unit.category == "K" &&
          this.to_unit.category == "D"
        ) {
          this.kd();
        } else {
      ...
      console.log(
        "Calculating " + this.from_unit.label + " to " + this.to_unit.label,
      );
    },
  },
  computed: {
