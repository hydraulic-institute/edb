Vue.component("friction-loss-calculator", {
  ...
  mounted: function () {
    ...
    if (typeof Storage !== "undefined") {
      ...
      console.log(
        "Local storage not available on this browser - unit sets will need to switch manually",
      );
    }

    axios
      .get("/statics/friction-loss-materials-full.json")
  ...
  methods: {
    ...
    head_loss: function (friction_factor, velocity_head) {
      return (
        ((friction_factor * this.input_length_feet * 12) /
          this.inner_diameter) *
        velocity_head
      );
    },
  ...
  computed: {
    ...
    specified: function () {
      return (
        this.entry != null &&
        (this.selector == null ||
          (this.selector != null && this.selection != null))
      );
    },
      ...
      for (const factor of steps) {
        ...
        if (Re < 2000) {
          ...
          const f2calc = function (f) {
            return (
              (-2 *
                Math.log(v.epsilon / (3.7 * D) + 2.51 / (Re * Math.sqrt(f)))) /
              Math.log(10)
            );
          };
          let f1 = f1calc(f);
          ...
          while (diff > 1 / 1000) {
            ...
            diff = f1 - f2;
            kcount++;
            if (!(kcount % 1000)) console.log(kcount + " ITERATIONS - " + diff);
          }
          console.log(kcount + " FINAL ITERATIONS");
          sample.friction_loss = f;
          sample.laminar = false;
        }
        sample.head_loss = this.head_loss(
          sample.friction_loss,
          sample.velocity_head,
        );
        ...
        // Laminar-Turbulent Transition Region (transition) is between 2000 and 4000,
        // so if the Reynolds number is in that range, mark it as a transition point.
        if (Re >= 2000 && Re <= 4000) {
...
Vue.component("mechanical-friction-loss-calculator", {
  ...
  methods: {
    calculate() {
      ...
      this.mech_friction_loss =
        (this.A * float_rpm_value + this.B) *
        Math.pow(float_shaft_diameter, this.EXP);
      if (this.bearing_spacing_5) {
        this.mech_friction_loss *= 2;
      }
      this.mech_friction_loss = this.mech_friction_loss * conversion;
      this.mech_friction = (this.mech_friction_loss * length_calc).toFixed(
        this.precision,
      );
...
Vue.component("atmospheric-pressure-calculator", {
  ...
  data: function () {
    return {
      data_table: {
        Pb: {
          metric: {val: 101325, units: "Pa"},
          us: {val: 29.9213, units: "inHg"},
          desc: "Reference pressure",
        },
        Tb: {
          metric: {val: 288.15, units: "K"},
          us: {val: 288.15, units: "K"},
          desc: "Reference temperature",
        },
        Lb: {
          metric: {val: -0.0065, units: "K/m"},
          us: {val: -0.00198, units: "K/ft"},
          desc: "Temperature lapse rate",
        },
        hb: {
          metric: {val: 0, units: "m"},
          us: {val: 0, units: "ft"},
          desc: "Height of reference pressure",
        },
        "R*": {
          metric: {val: 8.3144598, units: "J/(mol-K)"},
          us: {val: 89494.6, units: "lg-ft^2/(lb_mol-K-s^2)"},
          desc: "Universal gas constant",
        },
        g0: {
          metric: {val: 9.80665, units: "m/s^2"},
          us: {val: 32.1741, units: "ft/s^2"},
          desc: "Acceleration due to gravity",
        },
        M: {
          metric: {val: 0.0289644, units: "kg/mol"},
          us: {val: 28.9644, units: "lg/lg_mol"},
          desc: "Molar mass of earth's air",
        },
      },
      table_data: [],
      elevation_data: {
        //The order of the keys is the order they will appear in the table
        us: {
          mercury: {desc: "Inches of mercury (inHg)*", sea_level: "29.9"},
          pressure: {desc: "Pounds per square inch (psi)*", sea_level: "14.7"},
          atmosphere: {desc: "Atmospheres (atm)*", sea_level: "1.0"},
        ...
        metric: {
          ...
          atmosphere: {desc: "Bar*", sea_level: "1.0"},
      ...
      err_msg: "",
      max_elevation: {
        us: {val: 36000, str: "feet"},
        metric: {val: 10900, str: "meters"},
      },
    };
  },
  ...
  methods: {
    ...
    do_page_load: function () {
      ...
      this.calculate();
      this.max_elevation_str =
        this.max_elevation[this.page_units]["val"].toLocaleString() +
        " " +
        this.max_elevation[this.page_units]["str"];
    ...
    collectTableData: function () {
      ...
      for (var key in this.data_table) {
        let units_str = this.data_table[key][this.page_units][
          "units"
        ].replaceAll("^2", "\u00B2");
        this.table_data.push({
          var: key,
          val: this.data_table[key][this.page_units]["val"],
          units: units_str,
          desc: this.data_table[key]["desc"],
        });
      }
    },
    ...
    calculateValues: function () {
      ...
      let power_val =
        (-this.constants["g0"] * this.constants["M"]) /
        (this.constants["R*"] * this.constants["Lb"]);
      let calculation =
        this.constants["Pb"] *
        Math.pow(
          (this.constants["Tb"] +
            (this.elevation - this.constants["hb"]) * this.constants["Lb"]) /
            this.constants["Tb"],
          power_val,
        );
      if (this.page_units == "us") {
        out_obj["mercury"]["val"] = calculation;
        out_obj["pressure"]["val"] = this.fixed(
          out_obj["mercury"]["val"] * 0.4911541264,
        );
        out_obj["height"]["val"] = this.fixed(
          out_obj["mercury"]["val"] / 0.882647547,
        );
        out_obj["atmosphere"]["val"] = this.fixed(
          out_obj["mercury"]["val"] * 0.033421,
        );
      } else {
        out_obj["pressure"]["val"] = calculation / 1000;
        out_obj["mercury"]["val"] = this.fixed(
          out_obj["pressure"]["val"] / 0.133322047,
        );
        out_obj["height"]["val"] = this.fixed(
          out_obj["pressure"]["val"] * 0.101974787,
        );
        out_obj["atmosphere"]["val"] = this.fixed(
          out_obj["pressure"]["val"] * 0.01,
        );
      }
      for (var key in out_obj) {
        if (key == "error") continue;
        out_obj[key]["val"] = this.fixed_str(out_obj[key]["val"]);
        out_obj[key]["delta"] = this.fixed_str(
          this.fixed(this.elevation_data[this.page_units][key]["sea_level"]) -
            out_obj[key]["val"],
        );
      }
      console.log(
        "M: " +
          out_obj["mercury"]["val"] +
          " P:" +
          out_obj["pressure"]["val"] +
          " H:" +
          out_obj["height"]["val"] +
          " A:" +
          out_obj["atmosphere"]["val"],
      );
      console.log(
        "Md: " +
          out_obj["mercury"]["delta"] +
          " Pd:" +
          out_obj["pressure"]["delta"] +
          " Hd:" +
          out_obj["height"]["delta"] +
          " Ad:" +
          out_obj["atmosphere"]["delta"],
      );
      return out_obj;
    },
