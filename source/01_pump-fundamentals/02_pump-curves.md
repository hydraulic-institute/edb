...
A **pump performance curve** is a graphical representation of the head generated by a specific pump model at rates of flow from zero to maximum at a given operating speed. this seciton depicts curves for pump total head, efficinecy, input power, and net positive suction head required (NPSHR) as a function of flow rate. Equations are provide for power and efficiency related to flow rate, head and specific gravity. It should be noted that the liquid viscosity will effect the pump total head, flow, efficiency, NPSHR and power, which is not discussed here. Refer to <a href="https://www.pumps.org/what-we-do/standards/?pumps-search-product=9.6.7&hi-order=asc&hi-order-by=name" target="_blank">ANSI/HI 9.6.7</a> for effects of liquid viscosity on rotodynamic pump performance.

### Head and Flow Curve

The **head and flow curve** <units us = "is the most commonly used curve to describe pump performance. Pump total head (H) is plotted on the y-axis in <units us = "feet (ft)" metric = "meters (m)"/>. This is the measure of energy increase per unit weight of the liquid, imparted to the liquid by the pump, and is the difference between the total discharge head and the total suction head. On the x-axis is the flow rate, typically in <units us = "gallons per minute (GPM)" metric = "cubic meters per hour (m^3/h)"/>.
