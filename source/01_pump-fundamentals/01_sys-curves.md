...
Based on these concepts, the total system head at any given flow rate is the sum of the static head and frictional head losses in the system. It, therefore, can be represented using the following equation:
...
metric = "The following can be used to convert a flow rate in m^3^/h (cubic meters per hour) to a velocity in m/sec (with the pipe diameter D in meters)."/>
